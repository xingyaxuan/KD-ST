...
stu_net=CNN()
print(stu_net.state_dict())
stu_net.load_state_dict(torch.load("model/1cnn_stu_net.pth"))
...
for epoch in range(1, epoch + 1):
    ...
    if val_mae < best_val:
        with open('model/trans_1cnn_stu_model.pt', 'wb') as f:
            torch.save(stu_net, f)
        best_val = val_mae
with open('model/trans_1cnn_stu_model.pt', 'rb') as f:
...
csvfile = open("../compare_data_save/transfer_1cnn_1001.csv", 'wt', encoding="UTF8", newline='')  #
...
csvfile.close()
'''
csvfile = open("../compare_data_save/loss_trans_1dcnn.csv", 'wt', encoding="UTF8", newline='')  #
writer = csv.writer(csvfile, delimiter=",")
header = ['loss']
csvrow1 = []
